  ...
  def __init__(self, article, tags, abstract_sentences, abstract_sentences_all, vocab, hps, stop_words):
    ...
    if hps.pointer_gen:
      ...
      if hps.co_occurrence or hps.prev_relation or hps.co_occurrence_h or hps.co_occurrence_i or (hps.coverage and hps.coverage_weighted) or hps.attention_weighted or hps.markov_attention or hps.markov_attention_contribution:
        self.cooccurrence_matrix, self.cooccurrence_weight = data.get_cooccurrence_matrix(self.enc_input_extend_vocab, exclude_words=stop_words, need_weight=(hps.co_occurrence_i or (hps.coverage and hps.coverage_weighted) or hps.attention_weighted or hps.markov_attention or hps.markov_attention_contribution))
...
class Batcher(object):
  ...
  def __init__(self, data_path, vocab, hps, single_pass, stop_words):
    ...
    if self._hps.mode == 'decode':
      ...
      self._example_queue = Queue.Queue(self.BATCH_QUEUE_MAX * self._hps.batch_size * self._hps.max_keyphrase_num)
  ...
  def next_batch(self):
    ...
    if self._batch_queue.qsize() == 0:
      ...
      if self._single_pass and self._finished_reading and self._example_queue.qsize() == 0:
  ...
  def fill_example_queue(self):
    ...
    while True:
      ...
      if self._hps.mode == "decode":
        ...
        example = Example(article, tags, [abstract_sentences[0]], abstract_sentences, self._vocab, self._hps, self._stop_words) # Process into an Example.
        self._example_queue.put(example) # place the Example in the example queue.
  ...
  def fill_batch_queue(self):
    ...
    while True:
      if self._hps.mode != 'decode':
        ...
        if self._hps.co_occurrence or self._hps.prev_relation or self._hps.co_occurrence_h or self._hps.co_occurrence_i or (self._hps.coverage and self._hps.coverage_weighted) or self._hps.attention_weighted or self._hps.markov_attention or self._hps.markov_attention_contribution:
